...
XE에서 DB를 사용하려면 기본적으로 다음의 작업이 필요합니다.
...
XE에서 연동된 DB에 질의(쿼리)를 하려면 DB 테이블의 스키마를 먼저 정의해야 합니다. 'XML 스키마 정의 언어'(XML Schema Language)를 사용하여 정의할 수 있습니다.
...
또한, XE에서는 쿼리를 실행할 때 쿼리의 대상이 되는 테이블의 스키마 정보를 항상 참조합니다. 따라서 XE에서는 스키마가 정의되어 있지 않은 테이블은 사용할 수 없습니다.
